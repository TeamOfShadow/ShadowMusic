...
from ShadowMusic import BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from ShadowMusic.Database import (add_gban_user, add_off, add_on, add_sudo,
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m ShadowMusic")
        else:
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            ...
            return os.system(f"kill -9 {os.getpid()} && python3 -m ShadowMusic")
    ...
    for x in served_chats:
        try:
            await app.send_message(
                x,
                f"{MUSIC_BOT_NAME} has just restarted itself. Sorry for the issues.\n\nStart playing after 10-15 seconds again.",
            )
